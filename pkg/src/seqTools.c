...
SEXP count_dna_Kmers(SEXP pSeq,SEXP pStart,SEXP pWidth, SEXP pK,SEXP pNn)
{
	...
	for(i=0;i<nCols;++i)
	{
		 ...
		 * * * * * * * * * * * * * * * * * * * * * * * * * * */
		if(!do_countCheck_Kmers(dna+INTEGER(pStart)[i]-1,array+(i*nRows),INTEGER(pNn)+i,k,INTEGER(pWidth)[i]))
...
SEXP count_genome_Kmers(SEXP pSeq,SEXP pSeqid,SEXP pLstart,SEXP pWidth,SEXP pStrand, SEXP pK, SEXP pNn)
{
	...
	for(i=0;i<nCols;++i)
	{
		...
		{
			if(!do_rev_count_Kmers(dna+INTEGER(pLstart)[i]+INTEGER(pWidth)[i]-2,array+(i*nRows),INTEGER(pNn)+i,k,INTEGER(pWidth)[i]))
...
SEXP count_splice_Kmers(SEXP pSeq,SEXP pSeqid,SEXP pLend,SEXP pRstart,SEXP pWidth,SEXP pStrand, SEXP pK, SEXP pNn)
{
	...
	for(i=0;i<nCols;++i)
	{
		...
		{
			if(!do_rev_count_Kmers(dna+INTEGER(pRstart)[i]+INTEGER(pWidth)[i]-2,array+(i*nRows),INTEGER(pNn)+i,k,INTEGER(pWidth)[i]))
				++nMinusMm;
		}
	}
...
static R_INLINE int write_fastq_read(gzFile file,char *seq, char* phred, int seqlen, int *nWritten)
{
	/* * * * * * * * * * * * * * * * * * * * * * * * * * *
...
SEXP trim_fastq(SEXP pInfile, SEXP pVals, SEXP pOutfile)
{
	...
	while(!(fqpEmpty(fqp) || fqpFastqError(fqp)))
	{
		...
		*pEnd='\0';
		...
		if(discard)
		{
			if(write_fastq_read(discardfile,seq,phred,seqlen,&nDiscarded))
			{
				/*
		...
		*pEnd='\0';
		...
		if(seqlen<minSeqLen)
		{
			 ...
			 * Rprintf("[trim_fastq] Fix trim discard : Rest-len= %i < %i\n",seqlen,minSeqLen);
			 */
			if(write_fastq_read(discardfile,sBegin,pBegin,seqlen,&nDiscarded))
			{
				gzclose(keepfile);
				gzclose(discardfile);
				error("[trim_fastq] Write error: Read %u!\n",fqp->nSeq);
		...
		*pEnd='\0';
		 ...
		 */
		if(write_fastq_read(keepfile,sBegin,pBegin,seqlen,&nKept))
		{
			gzclose(keepfile);
			gzclose(discardfile);
...
{
	char *head,*buf,*buf_iter,*head_iter,*buf_end,*head_end;	//*seq,
